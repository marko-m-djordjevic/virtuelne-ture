(function () {
  ...
  function setActivePin(identifier) {
    document.querySelectorAll('.pin').forEach(function (pin) {
      pin.classList.toggle('pin-active', pin.getAttribute('data-identifier') === identifier);
    });
  }
  ...
  function handleDocumentClick(e) {
    ...
    if (mapWrapper && mapWrapper.classList.contains('menu-open') && !target.closest('.menu-sidebar') && !target.closest('.hamburger')) {
      mapWrapper.classList.remove('menu-open');
    }
    if (!isPinned || !popupOverlay.classList.contains('visible')) return;
    if (popup.contains(target) || target.closest('.pin') || target.closest('.menu-sidebar')) return;
    isPinned = false;
    hidePopup();
  }
  ...
  function init() {
    ...
    if (hamburger && mapWrapper) {
      hamburger.addEventListener('click', function (e) {
        ...
        hamburger.setAttribute('aria-expanded', mapWrapper.classList.contains('menu-open'));
        document.querySelector('.menu-sidebar').setAttribute('aria-hidden', !mapWrapper.classList.contains('menu-open'));
      });
    }
  }
